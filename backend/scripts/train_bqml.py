...
def train_model():
    # Train directly on `fin_clearing.fraud_transactions` using features that can be
    ...
    query = f"""
    CREATE OR REPLACE MODEL `{client.project}.{DATASET_ID}.fraud_model`
    OPTIONS(
    ...
    SELECT
        ...
        is_fraud
    FROM `fin_clearing.fraud_transactions`
    ...
    print(f"Model {DATASET_ID}.fraud_model successfully trained.")
...
def evaluate_model():
    ...
    SELECT
      *
    FROM
      ML.EVALUATE(MODEL `{client.project}.{DATASET_ID}.fraud_model`)
    """
